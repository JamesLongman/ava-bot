...
module.exports = {
    execute: function execute(message, details) {
        log.verbose(`Called by ${message.displayName} (${message.author.id}), on ${message.guild.name}` +
            ` (${message.guild.id})`);
        ...
        } else if (Object.prototype.hasOwnProperty.call(details(prefix), message.cleanContent.substring(6))) {
            // Recognised command/feature, reply with detailed explanation
            const cmd = message.cleanContent.substring(6);
            message.channel.sendMessage(`\`${prefix}${cmd}\` ${details(prefix)[cmd]}`)
