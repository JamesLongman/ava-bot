...
const commandsChannelCheck = require('./commandsChannel.js');
...
const uptimeMod = require('./commandModules/stats/uptime.js');

module.exports = {
    // Check that a command should be executed, return false if not
    check: function check(message) {
        /* Firstly check if a server has a command channel, if so then let the user know if they're attempting
        to use a command in another channel */
        if (!commandsChannelCheck.execute(message)) { return false; }

        // Finally check that a server is not exceeding its use limit
        // To do: Issue #17
        // if (!antispamCheck(message)) { return false; }
...
const commandFunctions = {
    ...
    uptime: function uptime(message) {
        return uptimeMod.execute(message);
    },
};
...
function statsEmbed(prefix) {
    return new discord.RichEmbed()
        .setAuthor('Ava', 'http://i.imgur.com/ZJEGUIk.jpg')
        .setColor('#0275ea')
        .setThumbnail('http://i.imgur.com/RDvjDQ0.jpg')
        .addField('__Stats Commands:__',
            `\`${prefix}uptime\` Posts a graph showing Ava bot's uptime`);
}
...
function uptimeEmbed(prefix) {
    return new discord.RichEmbed()
        .setAuthor('Ava', 'http://i.imgur.com/ZJEGUIk.jpg')
        .setTitle(`__Command:__ ${prefix}uptime`)
        .setColor('#0275ea')
        .setThumbnail('http://i.imgur.com/RDvjDQ0.jpg')
        .addField('Specifications',
            'Type: *Stats command*' +
            '\nPermissions Required: *None*')
        .addField('Description', 'Replies with statistics on Ava\'s uptime, the command will let you ' +
        'know: The uptime so far today, the uptime per day for the past week, the average uptime for ' +
        'the past month, and the average all-time uptime. This is calculated by recording each ' +
        'full uninterupted minute the bot is up. Please note that most trouble good discord bot\'s experience ' +
        'is caused by connections to the discord API which unfortunately is unavoidably unreliable at times')
        .addField('Use Example', `${prefix}uptime`);
}
